...
describe('Decks Routes', () => {
  ...
  describe('POST /decks', () => {
    it('devrait créer un nouveau deck avec succès', async () => {
      ...
      prismaMock.deck.create.mockResolvedValue(
        mockDeck as Deck & { deckCard: (DeckCard & { card: Card })[] },
      )
  ...
  describe('GET /decks/mine', () => {
    it("devrait retourner tous les decks de l'utilisateur", async () => {
      ...
      prismaMock.deck.findMany.mockResolvedValue(
        mockDecks as (Deck & { deckCard: DeckCard[] })[],
      )
      ...
      expect(response.body.decks).toMatchObject(
        mockDecks.map((deck) => ({
          id: deck.id,
          name: deck.name,
          userId: deck.userId,
    ...
    it("devrait retourner un message si aucun deck n'existe", async () => {
      ...
      expect(response.body.message).toBe(
        'Aucun deck trouvé pour cet utilisateur',
      )
    })
  ...
  describe('GET /decks/:id', () => {
    it('devrait retourner un deck spécifique', async () => {
      ...
      prismaMock.deck.findUnique.mockResolvedValue(
        mockDeck as Deck & { deckCard: DeckCard[] },
      )
  ...
  describe('PATCH /decks/:id', () => {
    ...
    it('devrait retourner 401 si le token est manquant', async () => {
      ...
      const response = await request(app).patch('/decks/1').send({
        name: 'Nouveau Nom',
      })
